...
import { getCurrentUserContact } from "../utils/finance";
		...
		<div className="grid  gap-4 px-4 py-8 lg:grid-cols-[auto,1fr]">
			<div className="mr-auto flex min-w-[300px] max-w-96 flex-col gap-3 rounded-xl border-2 border-black bg-blue-500 p-4 py-8 text-white shadow-lg shadow-blue-600">
				...
				<p className="text-4xl font-light">
					{getUserBalance(depositsQuery.data, transfersQuery.data, withdrawalsQuery.data)?.toLocaleString("en-US", {
						style: "currency",
						currency: "KES",
					})}
				</p>
			...
			<div className="flex w-full flex-col  justify-stretch overflow-hidden rounded-xl border-2 border-black bg-white transition-colors sm:flex-row  ">
				<Link to="/send" className="flex flex-1 flex-col items-center justify-center gap-4  px-2 py-12 hover:bg-yellow-100">
				...
				<Link to="/deposit" className="flex flex-1 flex-col items-center justify-center gap-4 px-2 py-12 hover:bg-green-100">
				...
				<Link to="/withdraw" className="flex flex-1 flex-col items-center justify-center gap-4 px-2 py-12 hover:bg-blue-100">
